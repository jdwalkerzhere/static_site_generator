...
class TestTextSplitting(unittest.TestCase):
    ...
    def test_split_expected(self):
        text_node = TextNode(
            text="Hello there *my* **name** `is jesse`", type=TextType.TEXT
        )
        expected_output = [
    ...
    def test_more_than_one_segment(self):
        text_node = TextNode(
            text="*hi* my *name* is `jesse` and *I* really **want** to get **good** at `programming`",
            type=TextType.TEXT,
        )
        expected_output = [
            TextNode(text="hi", type=TextType.ITALIC),
