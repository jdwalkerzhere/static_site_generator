...
class TestTextToLeafNode(unittest.TestCase):
    def test_text_node_conversion(self):
        text_node = TextNode(text="hi there",
                             type=TextType.TEXT,
                             url=None)
    ...
    def test_bold_italic_code_conversion(self):
        ...
        code_node = TextNode(text="python3 main.py",
                             type=TextType.CODE,
                             url=None)
    ...
    def test_link_conversion(self):
        node = TextNode(text='go learn to code',
                        type=TextType.LINK,
                        url='https://boot.dev')
        leaf = LeafNode(tag='a', value='go learn to code', props={'href': 'https://boot.dev'})
        self.assertEqual(text_node_to_html_node(node), leaf)

    def test_image_conversion(self):
        node = TextNode(text="I'm alt text!",
                        type=TextType.IMG,
                        url='https://boot.dev')
        leaf = LeafNode(tag='img', value="", props={'src': 'https://boot.dev', 'alt': "I'm alt text!"})
        self.assertEqual(text_node_to_html_node(node), leaf)
